...
function checkAnswer(selectedIndex) {
    ...
    if (selectedIndex === question.correct) {
        ...
        if (streak % streakBonusThreshold === 0) {
            ...
            document.getElementById("streak-message").innerHTML = `<strong>🔥 Amazing! Streak Bonus: +2 points</strong>`;
        } else if (streak > 1) {
            document.getElementById("streak-message").innerHTML = `<strong>🔥 Keep going! Streak: ${streak}</strong>`;
        }
    } else {
        streak = 0; // Reset streak on incorrect answer
        options[selectedIndex].classList.add("incorrect");
        document.getElementById("streak-message").innerHTML = `<strong>😢 Streak broken! Try again!</strong>`;
    }
...
function showEndScreen() {
    ...
    let finalStreak = document.getElementById("final-streak");
    finalStreak.innerText = streak; // Display the final streak
